...
import { colors, buttons, radius } from '../styles'
...
function NavLink({ to, label, icon, isActive }) {
  return (
    <Link key={to} to={to} style={{
      ...
      background: isActive ? colors.goldMuted : 'transparent',
...
function AdminModeButton({ isAdmin, adminMode, toggling, onToggle }) {
...
function AppPasswordIndicator({ isLoggedIn, appPasswordConfigured }) {
...
function UserSection({ userName, isAdmin, onLogout }) {
  return (
    <div style={{ 
      display: 'flex', 
      alignItems: 'center', 
    ...
    <nav style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '2.5rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.25rem' }}>
          {links.map(l => (
            <NavLink key={l.to} {...l} isActive={location.pathname === l.to} />
          ))}
        </div>
      </div>
